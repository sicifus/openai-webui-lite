    ...
    <script>
      ...
      class OpenaiDB {
        ...
        async testWebDAVConnection(config) {
          ...
          var headers = this._buildProxyHeaders(config, { Depth: '0' });
          try {
            var response = await fetch(proxyUrl, {
              method: 'PROPFIND',
              headers: headers
            });
            // 207 Multi-Status 表示成功, 404 可能是目录不存在需要创建
            if (response.status === 207 || response.status === 200) {
              return { success: true };
            } else if (response.status === 404) {
              // 尝试创建目录
              var mkcolResponse = await fetch(proxyUrl, {
                method: 'MKCOL',
                headers: this._buildProxyHeaders(config)
              });
              if (
              ...
              return {
                success: false,
                error: '无法创建目录: HTTP ' + mkcolResponse.status
              };
